...
export async function activate(ctx: ExtensionContext) {
	const languageServerBinary = workspace.getConfiguration().get('ginko.languageServer');
	...
	client = new LanguageClient('ginko_ls', 'Ginko LS', serverOptions, clientOptions);
...
async function getLatestLanguageServer(timeoutMs: number, ctx: ExtensionContext) {
	...
	if (semver.prerelease(latest)) {
		...
		const download = await fetch(browser_download_url, {
			...
			throw new Error(
				`Language server download timed out after ${timeoutMs.toFixed(2)} seconds.`
			);
		});
		...
		const fileStream = fs.createWriteStream(languageServerAsset, { flags: 'wx' });
